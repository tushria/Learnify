    ...
    if(action === "add"){
      ...
      if(title){
        const newChapter={
          ...
          chapterContent:[],
          collapsed:false,
          ChpaterOrder:chapters.length>0 ? chapters.slice(-1)[0].chapterOrder + 1:1,
        };
    ...
    setChapters(
      chapters.map((chapter)=>{
        if(chapter.chapterId ===  currentChapterId){
          const newLecture={
            ...lectureDetails,
            lectureOrder:chapterContent.length>0?chapter.AddCoursechapterContent.slice(-1)[0].lectureOrder + 1:1,
            lectureId:uniqid()
          };
      ...
      <form className='flex flex-col gap-4 max-w-md w-full text-gray-500' onSubmit={handleSubmit}>
        ...
        <div className='flex flex-col justify-evenly flex-wrap'>
          <div className='flex flex-col gap-1'>
            ...
            value={coursePrice} type="number" 
            placeholder='0' 
            className='outline-none md:py-2.5 py-2 w-28 rounded border border-gray-500' required />
          </div>
                    ...
                    <div className='p-4'>
                      {chapter.chapterContent.map((lecture, lectureIndex)=>(
                        <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                          <span>{lectureIndex+1} {lectureDetails.lectureTitle} - {lectureDetails.lectureDuration} mins - <a href={lecture.lectureUrl} target="_blank" className='text-blue-500 '>Link</a> - {lectureDetails.isPreviewFree ? "Free Preview" : "Paid"}</span>
